...
export async function notifyOps({ category, subject, body, context }) {
  try {
    ...
    // Defaults to vonpeach.com sender (domain verified on Resend). Override via RESEND_FROM env var.
    // We send ops alerts from "Ops <hello@vonpeach.com>" by default - gets clean delivery in your inbox.
    const fromAddr  = process.env.RESEND_FROM || 'Visibility Index Ops <hello@vonpeach.com>';
    if (!apiKey) {
