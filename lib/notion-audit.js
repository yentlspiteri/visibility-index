 ...
 *
 * Behaviour:
 *   - logAuditCompleted()   fires at the end of /api/score (status: audit_completed)
...
function buildAuditProperties(p) {
  ...
  if (p.tier)    props['Tier']    = { select: { name: String(p.tier).slice(0, 100) } };
...
export { logAuditCompleted, upsertAuditOnLead, upsertAuditOnBooking };
