...
export async function ensureSchema() {
  ...
  _schemaReady = (async () => {
    ...
    // ── Row-Level Security ──────────────────────────────────────────────
    // CRITICAL: Supabase exposes a public PostgREST endpoint at
    //   https://<project>.supabase.co/rest/v1/<table>
    // that anyone can hit with the publicly-known `anon` key. With RLS
    // OFF (the default), every row in every table is readable, editable
    // and deletable by anyone who knows the project URL. That URL appears
    // in env vars + occasional client-side network panels — assume it
    // leaks. The 3 token columns (manager.invite_token, magic_link.token,
    // consent_token.token) are live auth credentials; magic_link.token
    // grants login-as-any-manager. Catastrophic if leaked.
    //
    // Enabling RLS with NO POLICIES = deny by default for the `anon` and
    // `authenticated` roles. Our Vercel functions connect via POSTGRES_URL
    // as the `postgres` role, which bypasses RLS — so server-side reads
    // and writes keep working unchanged.
    //
    // Idempotent: ENABLE ROW LEVEL SECURITY is a no-op once set, so
    // running this on every cold start is safe and cheap.
    //
    // If we ever expose a public Supabase client (browser-side SDK), this
    // is also where we'd add explicit row-scoped policies (e.g. "a
    // manager can read only their own team_members"). Until then, the
    // policy-less deny-default is the right posture.
    await sql`ALTER TABLE manager        ENABLE ROW LEVEL SECURITY`;
    await sql`ALTER TABLE magic_link     ENABLE ROW LEVEL SECURITY`;
    await sql`ALTER TABLE team_member    ENABLE ROW LEVEL SECURITY`;
    await sql`ALTER TABLE score_snapshot ENABLE ROW LEVEL SECURITY`;
    await sql`ALTER TABLE consent_token  ENABLE ROW LEVEL SECURITY`;
  })();
  return _schemaReady;
}
