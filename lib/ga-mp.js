...
export async function sendGAMpEvent({ name, email, clientId, params = {}, userAgent, ip }) {
  ...
  // session_id must be STABLE across events from the same user within the
  // session window — otherwise every event looks like a fresh session and
  // GA can't attribute a source. We bucket into 30-minute windows (GA4's
  // default session timeout) so multiple events from the same visitor stay
  // grouped. The bucket is per-client because we mix it with the resolved
  // client_id below.
  const sessionBucket = Math.floor(Date.now() / (30 * 60 * 1000));
  const eventParams = Object.assign(
    {
      engagement_time_msec: 1,
      session_id:           String(sessionBucket) + '-' + resolvedClientId.slice(0, 8)
    },
    params
  );
