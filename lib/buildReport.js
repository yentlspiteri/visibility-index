...
function drawThreeMoves(ctx, r) {
  ...
  moves.forEach((m, i) => {
    ...
    // Solo column - now shows firstStep + week 1 milestone + success metric
    ...
    const stepEndY = drawWrapped(page, m.firstStep || 'Start with the smallest version this week.', {
      x: soloX, y: colY - 16, maxWidth: colW,
      size: 10, font: ctx.helv, color: COLOR.ink, lineHeight: 13
    });
    // Week 1 / Month 1 milestones if Claude returned them
    if (m.weekOne) {
      const w1Y = drawWrapped(page, '→ Week 1: ' + m.weekOne, {
        x: soloX, y: stepEndY - 14, maxWidth: colW,
        size: 9, font: ctx.helvObl, color: COLOR.indigo, lineHeight: 12
      });
      if (m.month1) {
        drawWrapped(page, '→ Month 1: ' + m.month1, {
          x: soloX, y: w1Y - 12, maxWidth: colW,
          size: 9, font: ctx.helvObl, color: COLOR.indigo, lineHeight: 12
        });
      }
    }
    ...
    const titleEndY = drawWrapped(page, svc.label, {
      x: helpX, y: colY - 16, maxWidth: colW,
      size: 11, font: ctx.helvBold, color: COLOR.indigo, lineHeight: 14
    });
    drawWrapped(page, svc.deck, {
      x: helpX, y: titleEndY - 12, maxWidth: colW,
      size: 10, font: ctx.helv, color: COLOR.ink, lineHeight: 13
    });
    ...
    if (m.successMetric) {
      drawWrapped(page, '✓ Success looks like: ' + m.successMetric, {
        x: MARGIN + 20, y: cardY + 18, maxWidth: CONTENT_W - 40,
        size: 9, font: ctx.helvObl, color: COLOR.grey, lineHeight: 12
      });
    }
...
function drawPlatformsPage(ctx, r) {
  ...
  detected.forEach((key) => {
    ...
    // Hit count badge (right)
    const hitCount = p.hits?.length || 0;
    if (hitCount > 0) {
      const badgeText = hitCount === 1 ? '1 result' : `${hitCount} results`;
      const badgeW = ctx.helv.widthOfTextAtSize(badgeText, 9) + 16;
      page.drawRectangle({
        x: PAGE_W - MARGIN - 22 - badgeW, y: y - 32, width: badgeW, height: 18,
        color: COLOR.indigo, opacity: 0.10
      });
      drawText(page, badgeText, {
        x: PAGE_W - MARGIN - 22 - badgeW + 8, y: y - 28, size: 9, font: ctx.helvBold, color: COLOR.indigo
      });
    }
    y = cardY - 12;
  });
