...
function drawWhereYouStand(ctx, r) {
  ...
  drawText(page, `Here's your shape, ${r.firstName || 'friend'}.`, {
...
function drawThreeMoves(ctx, r) {
  ...
  drawWrapped(page, `Three things, ${r.firstName || 'you'}. Pulled straight from what we saw.`, {
...
function drawTimeline(ctx, r) {
  ...
  drawText(page, `Your plan, ${r.firstName || 'you'}.`, {
    x: MARGIN, y: PAGE_H - 110, size: 32, font: ctx.helvBold, color: COLOR.cream
  });
  drawWrapped(page, "A simple 90-day plan. Some bits you can do yourself. The harder bits are where we come in.", {
...
function drawWhatWeDo(ctx, r) {
  ...
  drawWrapped(page, `Here's where we'd start, ${r.firstName || 'you'}. Sorted by what would help you most.`, {
...
function drawClosingCTA(ctx, r) {
  ...
  drawWrapped(page, "30 minutes. No pitch. We talk about what we saw, you ask questions, and we tell you what we would do first.", {
