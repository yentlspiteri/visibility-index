...
const LABELS = {
  en: {
    ...
    // 90-day Roadmap page. Every translatable string the page renders.
    // The roadmap content itself (focus, theme lines, weekly actions,
    // milestones, endState) is generated per-user by Claude — these are
    // the page chrome + fallbacks for when Claude's response was empty.
    roadmap: {
      eyebrow:           'YOUR 90-DAY ROADMAP',
      title:             'Where you go from here.',
      intro:             'Three months. One focus area. Twelve specific weekly actions to compound.',
      focusLabel:        'FOCUS',
      auditDateLabel:    'AUDIT DATE',
      targetDateLabel:   'TARGET DATE',
      monthBadge:        (n) => `MONTH ${n}`,
      themeLabel:        'THEME',
      weekLabel:         (n) => `WK ${n}`,
      milestoneLabel:    'MILESTONE',
      endStateEyebrow:   'BY DAY 90',
      endStateFallback:  'A sharper personal brand, a regular cadence, and one earned authority signal worth pointing at.',
      monthFallback: [
        { title: 'Foundation — close the biggest gap',
          theme: 'Fix the leak with the most leverage. The thing every later move will compound on.',
          weeks: [
            'Audit your weakest dimension. Name the single line you would change first.',
            'Make the change. Update headline, banner, or About to land the new positioning.',
            'Publish one post that proves the new positioning to your network.',
            'Repeat the post weekly with a small variation. Same theme, different angle.'
          ],
          milestone: 'A stranger reads your profile and writes one accurate sentence about what you do.'
        },
        { title: 'Rhythm — three pillars, weekly',
          theme: 'Stop posting at random. Build a rhythm your audience and the algorithm can recognise.',
          weeks: [
            'Pick three topic pillars. Write a one-line definition for each.',
            'Plan four posts across the three pillars for the next four weeks.',
            'Post twice this week. Note which one earned the most comments.',
            'Repeat. Adjust the underperforming pillar. Keep the others.'
          ],
          milestone: 'Three weeks of consistent posting across three named topic pillars.'
        },
        { title: 'Compound — earn your first signal',
          theme: 'Move from output to evidence. One earned signal beats ten posts about the same topic.',
          weeks: [
            'Pick one tier-1 outcome: press, podcast, or keynote pitch.',
            'Draft one pitch. Send it. Track the reply (or non-reply).',
            'Re-pitch a tighter version to a different recipient.',
            'Publish a post about what you learnt, even if the pitch did not land.'
          ],
          milestone: 'One earned signal in the public record — interview, mention, or stage time.'
        }
      ]
    }
  },
  de: {
    ...
    walkthroughCTA: 'Möchten Sie es gemeinsam durchgehen? Buchen Sie einen 15-minütigen 1:1-Readout >',
    // Page 3 — Six signals
    ...
    threeMovesSub: 'Drei. Konkret. Abgeleitet aus dem, was wir gesehen haben.',
    doThis: 'TUN SIE DIES',
    ...
    timelinePhases: [
      ...
      { title: 'Monat 2 - Produktion',     sub: 'Schaffen Sie Momente, die das Scrollen stoppen. Hier übernehmen wir den größten Teil der Arbeit.', tag: 'Mit FutureMakers',
        fallbackBody: 'Fotografie, Video, Banner-Refresh, Signature-Post-Format. Allein schwer, mit dem richtigen Team schnell.' },
      { title: 'Monat 3 - Aktivierung',    sub: 'Werden Sie sichtbar für die richtigen Gelegenheiten — mit konsistenter digitaler Präsenz und Botschaften, die authentisch zu Ihnen passen.', tag: 'Hybrid',
    ...
    // Page 5 — Trends
    trendsEyebrow: 'Schärferes Signal für 2026',
    ...
    trends: [
      { eyebrow: 'VIDEO',
        ...
        body: 'Vertikale Clips mit dem Gründer vor der Kamera, 30 bis 60 Sekunden, Hook in den ersten drei. Die Algorithmen 2026 bevorzugen Talking Heads vor gebrandeten Hochglanz-Reels — und das Publikum auch.',
        tryLabel: 'DIESE WOCHE PROBIEREN',
        tryLine: 'Nehmen Sie vier 45-Sekunden-Clips in einer Sitzung auf. Posten Sie einen pro Woche.',
        chip: 'EIN MONAT CONTENT AN EINEM STUDIOTAG',
        serviceKey: 'video' },
      { eyebrow: 'FOTOGRAFIE',
        headline: 'Das Porträt 2026 ist situativ — nicht aus dem Stockfoto-Katalog.',
    ...
    pressSummaryZero: (name) => `Wir haben ${name} bislang in keiner Tier-1-Presse gefunden. Genau das ist die Lücke, die wir schließen.`,
    ...
    bookCallBtn: '15-MIN-GESPRÄCH BUCHEN  >',
    // Footer
    footerWordmark: 'VON PEACH',
    footerSecond: '  ·  FutureMakers',
    ...
    dimPlain: {
      ...
      network:   'Ob Ihr Netzwerk wirklich weiß, wofür Sie stehen.'
    },
    dimShort: {
    ...
    services: {
      ...
      speaker:  { label: 'Bereit, den Raum zu füllen',
    ...
    // 90-Tage-Roadmap — Seiten-Chrome + Fallbacks für den Fall, dass
    // Claudes Response leer war. Inhaltliche Personalisierung kommt
    // von Claude (focus, theme, weekly actions, milestones, endState).
    roadmap: {
      eyebrow:           'IHRE 90-TAGE-ROADMAP',
      title:             'Wie es von hier weitergeht.',
      intro:             'Drei Monate. Ein Fokus. Zwölf konkrete wöchentliche Maßnahmen.',
      focusLabel:        'FOKUS',
      auditDateLabel:    'AUDIT-DATUM',
      targetDateLabel:   'ZIELDATUM',
      monthBadge:        (n) => `MONAT ${n}`,
      themeLabel:        'THEMA',
      weekLabel:         (n) => `WO ${n}`,
      milestoneLabel:    'MEILENSTEIN',
      endStateEyebrow:   'AM TAG 90',
      endStateFallback:  'Eine schärfere Personal Brand, eine regelmäßige Frequenz und ein verdientes Autoritätssignal, auf das Sie zeigen können.',
      monthFallback: [
        { title: 'Grundlagen — die größte Lücke schließen',
          theme: 'Schließen Sie die Lücke mit der größten Hebelwirkung. Das Fundament, auf dem alles Spätere aufbaut.',
          weeks: [
            'Prüfen Sie Ihre schwächste Dimension. Benennen Sie die eine Zeile, die Sie zuerst ändern würden.',
            'Setzen Sie die Änderung um. Aktualisieren Sie Headline, Banner oder About-Abschnitt.',
            'Veröffentlichen Sie einen Beitrag, der die neue Positionierung Ihrem Netzwerk vermittelt.',
            'Wiederholen Sie den Beitrag wöchentlich mit einer kleinen Variation. Gleiches Thema, andere Perspektive.'
          ],
          milestone: 'Eine fremde Person liest Ihr Profil und schreibt einen präzisen Satz über das, was Sie tun.'
        },
        { title: 'Rhythmus — drei Säulen, wöchentlich',
          theme: 'Hören Sie auf, zufällig zu posten. Bauen Sie einen Rhythmus, den Ihr Publikum und der Algorithmus erkennen.',
          weeks: [
            'Wählen Sie drei Themensäulen. Schreiben Sie eine Ein-Satz-Definition für jede.',
            'Planen Sie für die nächsten vier Wochen vier Beiträge über die drei Säulen.',
            'Posten Sie diese Woche zweimal. Notieren Sie, welcher Beitrag die meisten Kommentare erhielt.',
            'Wiederholen Sie. Passen Sie die schwache Säule an. Behalten Sie die anderen bei.'
          ],
          milestone: 'Drei Wochen konsistentes Posten über drei benannte Themensäulen.'
        },
        { title: 'Verstärken — das erste Signal verdienen',
          theme: 'Bewegen Sie sich von Output zu Beleg. Ein verdientes Signal schlägt zehn Beiträge zum selben Thema.',
          weeks: [
            'Wählen Sie ein Tier-1-Ergebnis: Presse, Podcast oder Keynote-Pitch.',
            'Entwerfen Sie einen Pitch. Senden Sie ihn. Tracken Sie die Antwort (oder das Schweigen).',
            'Pitchen Sie eine schärfere Version an einen anderen Empfänger.',
            'Veröffentlichen Sie einen Beitrag darüber, was Sie gelernt haben — auch wenn der Pitch nicht ankam.'
          ],
          milestone: 'Ein verdientes Signal im öffentlichen Raum — Interview, Erwähnung oder Bühnenauftritt.'
        }
      ]
    }
  }
...
function sanitizeReport(r) {
  ...
  // 90-day roadmap — sanitize focus + endState + every month/week/milestone
  // so an emoji or rare unicode glyph from Claude can't crash pdf-lib's
  // glyph table. Same defensive pattern as moves above.
  if (out.roadmap && typeof out.roadmap === 'object') {
    out.roadmap = {
      focus:    sanitizeText(out.roadmap.focus),
      endState: sanitizeText(out.roadmap.endState),
      months:   Array.isArray(out.roadmap.months) ? out.roadmap.months.map(m => ({
        title:     sanitizeText(m?.title),
        theme:     sanitizeText(m?.theme),
        weeks:     Array.isArray(m?.weeks) ? m.weeks.map(sanitizeText) : [],
        milestone: sanitizeText(m?.milestone)
      })) : []
    };
  }
...
export async function buildReportPDF(rawReport = {}) {
  ...
  // 90-day Roadmap — a dedicated structured-plan page modelled on the
  // mentorship-roadmap format Yentl shared. Three months × four weekly
  // actions × milestone gate, plus a focus header and end-state footer.
  // Personalised by Claude (r.roadmap); falls back to dimension-aware
  // canned content when Claude's response was empty.
  drawRoadmapPage(ctx, r);
...
/* ───────────────────────────── PAGE 5 - 90-DAY ROADMAP ─────────────────────────────
   A dedicated roadmap page modelled on the structured-mentorship format
   the user shared (Jorgo Kapouranis's 6-month student roadmap). Three
   monthly sections × four weekly actions × per-month milestone gate, plus
   a focus header (audit date → target date) and an end-state footer line.

   Content sourced from Claude (r.roadmap.{focus,endState,months[].theme,
   .weeks[], .milestone}) and chrome/labels from LABELS[lang].roadmap.
   When Claude returns null/empty (rate-limit or parse fail), we fall back
   to LABELS[lang].roadmap.monthFallback so the page still ships a useful
   default rather than blanking. The fallbacks are dimension-aware: they
   tell the user to fix their weakest dimension first.

   Layout (A4 portrait, 56px margin all sides):
     - Eyebrow + title + intro      (≈100px)
     - Focus card with audit→target ( ≈70px)
     - 3 month cards stacked        (3 × ≈180px = 540px)
     - End-state footer             (≈40px)
     Total ≈ 760px, fits 730px content area with tight spacing. */
function drawRoadmapPage(ctx, r) {
  const L = ctx.L;
  // Defensive: if roadmap labels weren't loaded (unknown lang), bail —
  // worse to render half a page than to throw.
  if (!L.roadmap) {
    console.warn('[buildReport] L.roadmap missing for lang=' + ctx.lang + '; skipping roadmap page');
    return;
  }
  const RM = L.roadmap;
  const page = ctx.pdf.addPage([PAGE_W, PAGE_H]);
  ...
  // Use Claude's roadmap if present; otherwise the per-language fallback.
  // Both shapes converge on { focus, endState, months[{title,theme,weeks[],milestone}] }
  // so the downstream layout code is identical.
  const rm = (r.roadmap && Array.isArray(r.roadmap.months) && r.roadmap.months.length === 3)
    ? r.roadmap
    : {
        focus:    pickFallbackFocus(r, ctx.lang),
        endState: RM.endStateFallback,
        months:   RM.monthFallback
      };

  // Compute the 90-day target date string. Localised: 'Sep 6 2026' for EN,
  // '6. Sept. 2026' for DE. Renders both today and target on the same row.
  const today = new Date();
  const target = new Date(today.getTime());
  target.setDate(target.getDate() + 90);
  const auditDateStr  = formatDateShort(today,  ctx.lang);
  const targetDateStr = formatDateShort(target, ctx.lang);

  // ── EYEBROW + TITLE + INTRO ───────────────────────────────────────────
  drawSectionEyebrow(ctx, page, RM.eyebrow, PAGE_H - 60);
  drawText(page, RM.title, {
    x: MARGIN, y: PAGE_H - 110, size: 32, font: ctx.helvBold, color: COLOR.black
  });
  drawWrapped(page, RM.intro, {
    x: MARGIN, y: PAGE_H - 145, maxWidth: CONTENT_W - 60,
    size: 11, font: ctx.helv, color: COLOR.ink, lineHeight: 15
  });

  // ── FOCUS CARD ────────────────────────────────────────────────────────
  // Slim 3-column row: FOCUS | AUDIT DATE | TARGET DATE.
  // Stays on a single line even for long focus names (sanitised at the
  // 120-char slice in api/score.js).
  const focusCardY = PAGE_H - 200;
  const focusCardH = 64;
  page.drawRectangle({
    x: MARGIN, y: focusCardY - focusCardH, width: CONTENT_W, height: focusCardH,
    color: COLOR.cream,
    borderColor: COLOR.indigo, borderWidth: 1.2
  });
  // 3 columns at fixed widths so the layout is stable regardless of
  // focus-text length: 55% focus, 22.5% audit date, 22.5% target date.
  const col1X = MARGIN + 18;
  const col2X = MARGIN + CONTENT_W * 0.55;
  const col3X = MARGIN + CONTENT_W * 0.775;
  drawText(page, RM.focusLabel, {
    x: col1X, y: focusCardY - 22, size: 7.5, font: ctx.helvBold, color: COLOR.indigo, characterSpacing: 1.4
  });
  drawText(page, (rm.focus || '—').toUpperCase(), {
    x: col1X, y: focusCardY - 44, size: 13, font: ctx.helvBold, color: COLOR.black
  });
  drawText(page, RM.auditDateLabel, {
    x: col2X, y: focusCardY - 22, size: 7.5, font: ctx.helvBold, color: COLOR.indigo, characterSpacing: 1.4
  });
  drawText(page, auditDateStr, {
    x: col2X, y: focusCardY - 44, size: 13, font: ctx.helvBold, color: COLOR.black
  });
  drawText(page, RM.targetDateLabel, {
    x: col3X, y: focusCardY - 22, size: 7.5, font: ctx.helvBold, color: COLOR.indigo, characterSpacing: 1.4
  });
  drawText(page, targetDateStr, {
    x: col3X, y: focusCardY - 44, size: 13, font: ctx.helvBold, color: COLOR.black
  });

  // ── 3 MONTH CARDS ─────────────────────────────────────────────────────
  // Stacked vertically. Each card has a fixed height so the page layout
  // is predictable across reports — overflow is clamped by drawWrappedClamped.
  let y = focusCardY - focusCardH - 20;
  const cardH = 170;
  const cardGap = 12;
  rm.months.forEach((m, idx) => {
    const cardY = y - cardH;
    // Card background — subtle indigo tint, deliberate progression so the
    // reader sees the months as ascending intensity.
    const tint = idx === 2 ? 0.06 : idx === 1 ? 0.04 : 0.025;
    page.drawRectangle({
      x: MARGIN, y: cardY, width: CONTENT_W, height: cardH,
      color: rgb(0.247, 0.211, 0.698), opacity: tint,
      borderColor: COLOR.indigo, borderWidth: 0.6
    });

    // MONTH N badge — top-left
    const badgeText = RM.monthBadge(idx + 1);
    const badgeW = safeWidth(ctx.helvBold, badgeText, 8) + 24;
    page.drawRectangle({
      x: MARGIN + 14, y: y - 24, width: badgeW, height: 18,
      color: COLOR.indigo
    });
    drawText(page, badgeText, {
      x: MARGIN + 14 + 12, y: y - 18, size: 8, font: ctx.helvBold,
      color: COLOR.cream, characterSpacing: 1.4
    });
    // Month title — right of badge
    drawText(page, m.title || RM.monthFallback[idx].title, {
      x: MARGIN + 14 + badgeW + 12, y: y - 19, size: 13, font: ctx.helvBold,
      color: COLOR.black
    });
    // Theme line — wraps to 1-2 lines under the title row
    drawWrappedClamped(page, m.theme || RM.monthFallback[idx].theme, {
      x: MARGIN + 16, y: y - 45, maxWidth: CONTENT_W - 32,
      size: 10, font: ctx.helvObl, color: COLOR.ink, lineHeight: 13, maxLines: 2
    });

    // 4 weekly action rows — each prefixed with "WK n  ·" pill, single-line clamp
    const weeks = m.weeks && m.weeks.length === 4 ? m.weeks : RM.monthFallback[idx].weeks;
    const weeksStartY = y - 74;
    weeks.forEach((w, wi) => {
      const wy = weeksStartY - wi * 16;
      // Week label pill
      const wkLabel = RM.weekLabel(idx * 4 + wi + 1);
      drawText(page, wkLabel, {
        x: MARGIN + 16, y: wy, size: 7.5, font: ctx.helvBold,
        color: COLOR.indigo, characterSpacing: 1.0
      });
      // Action text — clamped to a single line
      drawWrappedClamped(page, w, {
        x: MARGIN + 50, y: wy, maxWidth: CONTENT_W - 70,
        size: 9.5, font: ctx.helv, color: COLOR.black, lineHeight: 12, maxLines: 1
      });
    });

    // Milestone bar — bottom strip of the card
    const msY = cardY + 18;
    page.drawLine({
      start: { x: MARGIN + 16, y: msY + 12 },
      end:   { x: MARGIN + CONTENT_W - 16, y: msY + 12 },
      thickness: 0.4,
      color: COLOR.indigo,
      opacity: 0.25
    });
    drawText(page, RM.milestoneLabel, {
      x: MARGIN + 16, y: msY, size: 7.5, font: ctx.helvBold,
      color: COLOR.indigo, characterSpacing: 1.2
    });
    drawWrappedClamped(page, m.milestone || RM.monthFallback[idx].milestone, {
      x: MARGIN + 80, y: msY, maxWidth: CONTENT_W - 100,
      size: 9, font: ctx.helvObl, color: COLOR.black, lineHeight: 12, maxLines: 1
    });
  ...
  // ── END-STATE FOOTER ──────────────────────────────────────────────────
  // The "by day 90" line — concise statement of where the user will be if
  // they execute. Sets up the closing-CTA page that follows.
  const endY = y;
  drawText(page, RM.endStateEyebrow, {
    x: MARGIN, y: endY - 4, size: 8, font: ctx.helvBold,
    color: COLOR.indigo, characterSpacing: 1.6
  });
  drawWrappedClamped(page, rm.endState || RM.endStateFallback, {
    x: MARGIN, y: endY - 22, maxWidth: CONTENT_W,
    size: 11, font: ctx.helvObl, color: COLOR.black, lineHeight: 15, maxLines: 2
  });

  drawPageFooter(ctx, page, 5);
}

/* Helper: locale-aware short date for the focus card.
   EN: "Sep 6, 2026"   DE: "6. Sept. 2026" (matches the German convention
   the rest of the report uses for dates). */
function formatDateShort(d, lang) {
  try {
    ...
    // Defensive fallback for Node versions without full Intl data.
...
/* Helper: when Claude's roadmap is missing, derive the focus dimension
   from the user's lowest sub-score so even the fallback feels personalised
   ("FOKUS: MARKENKLARHEIT" — not just "FOKUS: -"). */
function pickFallbackFocus(r, lang) {
  const subs = r.subs || {};
  const dimKeys = ['footprint','clarity','authority','cadence','visual','network'];
  let lowestKey = 'clarity';
  let lowestScore = Infinity;
  dimKeys.forEach(k => {
    const v = Number(subs[k]);
    if (!isNaN(v) && v < lowestScore) { lowestScore = v; lowestKey = k; }
  });
  const L = LABELS[lang] || LABELS.en;
  return (L.dimLabels && L.dimLabels[lowestKey]) || lowestKey;
}
