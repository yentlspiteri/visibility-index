...
function drawThreeMoves(ctx, r) {
  ...
  moves.forEach((m, i) => {
    ...
    // Where we'd help column. Service labels are now outcome-led sentences (longer
    // than the old "Brand strategy" / "PR advisory" tokens), so we wrap them and let
    // the deck flow underneath the wrapped title.
    const svc = SERVICES[m.service] || SERVICES.strategy;
    const helpW = (CONTENT_W - 80) / 2 - 12;
...
function drawClosingCTA(ctx, r) {
  ...
  drawWrapped(page, "30 minutes, no pitch. We'll break down what we observed, take your questions and identify the quick wins.", {
  ...
  const btnW = 360;
  const btnH = 60;
  const calendlyUrl = 'https://calendly.com/yentl-spiteri/30min';
  page.drawRectangle({
