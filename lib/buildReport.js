...
async function loadAileron(pdf) {
  ...
  // `subset: true` keeps the PDF small (only embeds glyphs we actually use)
  const reg    = await pdf.embedFont(regBytes,    { subset: true });
  const bold   = await pdf.embedFont(boldBytes,   { subset: true });
  const black  = await pdf.embedFont(blackBytes,  { subset: true });
...
export async function buildReportPDF(rawReport) {
  ...
  // Try Aileron from Fonts/ first; fall back to Helvetica if anything fails (CI, missing files,
  // or hang during readFile on Vercel — we cap at 3 seconds to never block the function).
  let helv, helvBold, helvObl, helvBlack;
  const fontTimeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error('Aileron load timeout (3s)')), 3000)
  );
  try {
    const ail = await Promise.race([loadAileron(pdf), fontTimeout]);
    helv = ail.reg; helvBold = ail.bold; helvObl = ail.italic; helvBlack = ail.black;
    console.log('[buildReport] Aileron loaded OK');
  } catch (err) {
    console.warn('[buildReport] Aileron not loaded, falling back to Helvetica:', err?.message || err);
    helv      = await pdf.embedFont(StandardFonts.Helvetica);
    helvBold  = await pdf.embedFont(StandardFonts.HelveticaBold);
    helvObl   = await pdf.embedFont(StandardFonts.HelveticaOblique);
    helvBlack = helvBold;
  }
