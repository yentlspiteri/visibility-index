...
export async function buildReportPDF(rawReport) {
  ...
  // Aileron is OPT-IN via env var until we confirm fontkit + OTF embedding is stable on Vercel.
  // Set USE_AILERON_PDF=1 to enable; default is Helvetica (proven, fast, never hangs).
  let helv, helvBold, helvObl, helvBlack;
  if (process.env.USE_AILERON_PDF === '1') {
    ...
    try {
      ...
      console.warn('[buildReport] Aileron failed, using Helvetica:', err?.message || err);
    ...
    // Helvetica path - no fontkit, no readFile, no subset operations. Boring and reliable.
    helv      = await pdf.embedFont(StandardFonts.Helvetica);
    helvBold  = await pdf.embedFont(StandardFonts.HelveticaBold);
    helvObl   = await pdf.embedFont(StandardFonts.HelveticaOblique);
    helvBlack = helvBold;
