...
/* ─────────────────────────────────────── PAGE 1 — COVER ───────────────────────────────────────
   Editorial cover, deck-style. The composite score is the dominant element
   (240pt cream, off-axis), with the user's name as a small caption above and
   the tier as a quiet line below. Mirrors a presentation title slide — single
   idea, big type, lots of breathing room. */
function drawCover(ctx, r) {
  ...
  // Personal name — set as a deck-style cover lockup. Small "FOR" label sits
  // above a bold cream first-name line so the reader sees their own name
  // immediately, before the score.
  const firstName = r.firstName || 'You';
  drawText(page, 'FOR', {
    x: MARGIN, y: PAGE_H - 95, size: 10, font: ctx.helvBold,
    color: COLOR.lavender, characterSpacing: 2.4
  });
  drawText(page, firstName, {
    x: MARGIN, y: PAGE_H - 130, size: 36, font: ctx.helvBold,
    color: COLOR.cream, characterSpacing: -0.4
  });

  // ── Hero score: full-bleed centre treatment ──
  const display = Math.round(((r.total ?? 0) / 18) * 100);
  const scoreSize = 240;
  const scoreW = ctx.helvBold.widthOfTextAtSize(String(display), scoreSize);
  // Position the score so the bottom of the digits sits roughly at vertical middle
  const scoreY = PAGE_H / 2 - scoreSize * 0.22;
  ...
  // Tiny "/ 100" right-aligned below the score baseline
  drawText(page, 'out of 100', {
    x: (PAGE_W - ctx.helv.widthOfTextAtSize('out of 100', 12)) / 2,
    y: scoreY - 18,
    size: 12, font: ctx.helv, color: rgb(0.7, 0.7, 0.85), characterSpacing: 0.6
  });

  // Tier — single line, lavender, centred
  const tierLabel = r.tier?.name || '';
  drawText(page, tierLabel, {
    x: (PAGE_W - ctx.helvBold.widthOfTextAtSize(tierLabel, 18)) / 2,
    y: 240, size: 18, font: ctx.helvBold, color: COLOR.lavender
  });
  // Tier tagline — quiet italic underline
  const tagline = r.tier?.tagline || '';
  if (tagline) {
    const tagW = ctx.helvObl.widthOfTextAtSize(tagline, 12);
    drawText(page, tagline, {
      x: (PAGE_W - tagW) / 2,
      y: 218, size: 12, font: ctx.helvObl, color: rgb(0.78, 0.78, 0.92)
    });
  }

  // Bottom signature band — slim, deck-style
  page.drawLine({
    start: { x: MARGIN, y: 70 }, end: { x: PAGE_W - MARGIN, y: 70 },
    thickness: 0.5, color: rgb(0.30, 0.27, 0.55)
  });
  drawText(page, 'futuremakers', {
    x: MARGIN, y: 50, size: 9, font: ctx.helvBold, color: COLOR.lavender, characterSpacing: 1.2
  });
  drawText(page, '01', {
    x: PAGE_W - MARGIN - 18, y: 50, size: 9, font: ctx.helvBold, color: COLOR.lavender
  });
}
...
function drawWhereYouStand(ctx, r) {
  ...
  // Executive summary as a deck-style pull quote — lavender side rule + larger
  // italic body. Reads as a takeaway, not a paragraph buried in the page.
  const summary = r.executiveSummary || `${r.firstName || 'You'}, you have real signals to build on. Here's what to tighten next.`;
  const summaryY = hexCy - hexR - 40;
  // Lavender 3px side rule running the height of the quote
  page.drawRectangle({
    x: MARGIN, y: summaryY - 56, width: 3, height: 70,
    color: COLOR.lavender
  });
  drawWrappedClamped(page, summary, {
    x: MARGIN + 18, y: summaryY, maxWidth: CONTENT_W - 24,
    size: 15, font: ctx.helvObl, color: COLOR.black, lineHeight: 22,
    maxLines: 3
  });
...
function drawThreeMoves(ctx, r) {
  ...
  moves.forEach((m, i) => {
    // Card height tightened to 195. Each card now has explicit zones (no more
    // overlapping flex content). 3 × 207px + 120px title block = 741px — fits A4.
    const cardH = 195;
    const cardY = y - cardH;
    page.drawRectangle({
    ...
    // ─── Header zone (y - 0 to y - 36) ───
    drawText(page, '0' + (i + 1), {
      x: MARGIN + 20, y: y - 26, size: 12, font: ctx.helvBold, color: COLOR.indigo, characterSpacing: 1.4
    });
    // Reserve space for the right-side time-invest chip so long Claude-generated
    // titles (e.g. "Rewrite headline to signal investment thesis" for Bill
    // Gates) don't render into it. Truncate by measured width — char-count
    // truncation can't catch this because chars vary in width.
    ...
    // ─── Why zone (y - 44 to y - 76, hard 2-line clamp) ───
    // drawWrappedClamped enforces maxLines + ellipsises. Plain drawWrapped
    // had no maxLines so long Claude whys (200+ chars on high-tier profiles)
    // overflowed into the column zone below.
    ...
    // ─── Two-column body (y - 86 to y - 162, fixed 76px tall) ───
    // Each column has: eyebrow (12px) + ONE-line firstStep/svc-label (12px) +
    // ONE-line milestone/svc-deck (12px). 3 × 12 = 36px content height.
    ...
    // Week 1 milestone — single line, truncated. (Month 1 dropped from page 4
    // to prevent the bottom-overlap that was happening with full text.)
    if (m.weekOne) {
      drawText(page, '> ' + truncate(m.weekOne, 58), {
        x: soloX, y: colY - 44, size: 8.5, font: ctx.helvObl, color: COLOR.indigo
      });
    }
    ...
    drawText(page, truncate(svc.label, 60), {
      x: helpX, y: colY - 14, size: 10.5, font: ctx.helvBold, color: COLOR.indigo
    });
    // Clamped to 2 lines so the deck text never bleeds past the card bottom
    // or off the right edge (was rendering "An investor content plan that
    // build…" on Bill's report — text was wrapping to a 4th line that got
    // visually clipped by the success-metric strip below).
    drawWrappedClamped(page, svc.deck, {
      x: helpX, y: colY - 30, maxWidth: colW,
      size: 9.5, font: ctx.helv, color: COLOR.ink, lineHeight: 12,
      maxLines: 2
    });

    // ─── Success metric strip at bottom of card (y zone: cardY+8 to cardY+22) ───
    // Has its OWN dedicated 14px height — content above ends at cardY+30 so no overlap.
    if (m.successMetric) {
      // Subtle separator above the success line so it reads as its own zone
      page.drawLine({
        start: { x: MARGIN + 20, y: cardY + 26 },
        end:   { x: PAGE_W - MARGIN - 20, y: cardY + 26 },
        thickness: 0.4,
        color: rgb(0.88, 0.88, 0.92)
      });
      drawText(page, 'Success: ' + truncate(m.successMetric, 110), {
        x: MARGIN + 20, y: cardY + 12, size: 8.5, font: ctx.helvObl, color: COLOR.grey
      });
    }
...
function drawTrendsAndTactics(ctx, r) {
  ...
  TRENDS.forEach((t, i) => {
    ...
    // Body — wrapped, 2 lines max at this width/size
    drawWrapped(page, t.body, {
      x: MARGIN + 20, y: y - 78, maxWidth: CONTENT_W - 40,
      size: 11, font: ctx.helv, color: COLOR.ink, lineHeight: 15
    });

    // "Try this week" line — italic indigo
    drawText(page, t.tryLabel, {
      x: MARGIN + 20, y: cardY + 56, size: 8, font: ctx.helvBold, color: COLOR.indigo, characterSpacing: 1.4
    });
    drawWrapped(page, t.tryLine, {
      x: MARGIN + 20, y: cardY + 42, maxWidth: CONTENT_W - 40,
      size: 11, font: ctx.helvObl, color: COLOR.black, lineHeight: 14
    });

    // Service chip — lavender pill, bottom right of card
    const svc = SERVICES[t.serviceKey];
    const chipPad = 12;
    const chipText = t.chip;
    const chipTextW = ctx.helvBold.widthOfTextAtSize(chipText, 8);
    const chipW = chipTextW + chipPad * 2;
    const chipH = 20;
    const chipX = MARGIN + CONTENT_W - 20 - chipW;
    const chipY = cardY + 14;
    page.drawRectangle({
      x: chipX, y: chipY, width: chipW, height: chipH,
      color: COLOR.lavender, opacity: 0.18,
      borderColor: COLOR.lavender, borderWidth: 0.8
    });
    drawText(page, chipText, {
      x: chipX + chipPad, y: chipY + 7, size: 8, font: ctx.helvBold,
      color: COLOR.indigo, characterSpacing: 1.2
    });
    // Subtle "with us" left of the chip (caption only — chip carries the action)
    drawText(page, 'WITH US', {
      x: MARGIN + 20, y: chipY + 7, size: 8, font: ctx.helvBold,
...
function drawContentIdeasPage(ctx, r) {
  ...
  // Page numbers passed explicitly so the footer renders "06"/"07" instead
  // of the literal string "undefined" (was visible on Bill's report).
  if (writes.length > 0) drawIdeaPage(ctx, r, writes, {
    eyebrow: 'WHAT TO WRITE',
    title:   'Posts that would land for you.',
    sub:     `${r.firstName || 'You'}, three prompts straight from your audit.`,
    kind:    'write',
    pageNum: 6
  });

  if (films.length > 0) drawIdeaPage(ctx, r, films, {
    eyebrow: 'WHAT TO FILM',
    title:   'Videos you could shoot this week.',
    sub:     'Three ideas. Each shootable on a phone in under two hours.',
    kind:    'video',
    pageNum: 7
  });
}
...
function drawIdeaPage(ctx, r, ideas, opts) {
  ...
  ideas.slice(0, 3).forEach((idea, i) => {
    ...
    // Prefer the new schema (title/hook/angle/format/shotIn). Fall back to legacy topic/angle/format.
    ...
    const cardH  = (hook ? 50 : 0) + (angle ? 40 : 0) + (format ? 22 : 0) + 60;
    page.drawRectangle({
      x: MARGIN, y: y - cardH, width: CONTENT_W, height: cardH,
      color: rgb(1, 1, 1), borderColor: rgb(0.85, 0.85, 0.92), borderWidth: 1
    });
    ...
    let lineY = y - 54;
    if (hook) {
      drawWrapped(page, '"' + hook + '"', {
        x: MARGIN + 50, y: lineY, maxWidth: CONTENT_W - 80,
        size: 11, font: ctx.helvObl, color: COLOR.indigo, lineHeight: 15
      });
      lineY -= 36;
    }
    if (angle && angle !== title) {
    ...
    if (format) {
      ...
      drawText(page, metaParts.join('  ·  '), {
        x: MARGIN + 50, y: y - cardH + 18, size: 10, font: ctx.helvBold, color: COLOR.indigo, characterSpacing: 0.5
      });
    }
    y -= cardH + 14;
  });
...
function drawClosingCTA(ctx, r) {
  ...
  // Calendly CTA — rendered as a true visual button (filled lavender background,
  // dark outer border for definition, single bold label, dropped the secondary
  // URL line that was making it look like a link list). The whole rect is one
  // PDF link annotation so any click-through opens Calendly.
  ...
  // Subtle lavender shadow underneath for depth (rendered first, behind button)
  page.drawRectangle({
    x: MARGIN + 2, y: btnY - 4, width: btnW, height: btnH,
    color: rgb(0.45, 0.40, 0.85), opacity: 0.35
  });
  // Main button fill — cream on navy reads as a strong CTA
  page.drawRectangle({
  ...
  const label = 'BOOK A 15-MIN CALL  >';
  const labelSize = 14;
  ...
  // Whole-button click annotation
  addLinkAnnotation(ctx, page, calendlyUrl, {
    x: MARGIN, y: btnY, width: btnW, height: btnH
  });
  // Small "or copy: ..." line BELOW the button so the URL is still grabbable
  drawText(page, 'or copy: calendly.com/yentl-spiteri/15-min-intro', {
    x: MARGIN, y: btnY - 22, size: 9, font: ctx.helv, color: rgb(0.78, 0.78, 0.88)
  });
