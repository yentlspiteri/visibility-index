...
function drawRoadmapPage(ctx, r) {
  ...
  roadmap.forEach((item) => {
    // Use a triangle bullet (U+25B6, NOT WinAnsi) → fall back to ASCII arrow ">"
    drawText(page, '>', {
...
function drawText(page, text, opts) {
  ...
  // Final WinAnsi safety net — every literal in this file goes through here, so a stray
  // unicode char in any hardcoded string can't crash the PDF build.
...
function drawWrapped(page, text, opts) {
  ...
  // Sanitize at the source — same WinAnsi safety as drawText
