...
function drawThreeMoves(ctx, r) {
  ...
  moves.forEach((m, i) => {
    ...
    drawText(page, truncate(m.title, 60), {
    ...
    // ─── Why zone (y - 44 to y - 76, max 2 lines) ───
    // Truncated so it never overflows into the column zone below.
    drawWrapped(page, truncate(m.why, 180), {
    ...
    drawWrapped(page, truncate(m.firstStep || 'Start with the smallest version this week.', 130), {
    ...
    drawWrapped(page, truncate(svc.deck, 110), {
...
function drawTrendsAndTactics(ctx, r) {
  ...
  drawText(page, 'what the strongest brands are doing.', {
    x: MARGIN, y: PAGE_H - 115, size: 32, font: ctx.helvBold, color: COLOR.black, characterSpacing: -0.3
  });
  drawWrapped(page, 'Three trends shaping the next 12 months — and one move you can make this week.', {
...
function drawContentIdeasPage(ctx, r) {
  ...
  if (writes.length > 0) drawIdeaPage(ctx, r, writes, {
    ...
    sub:     `${r.firstName || 'You'}, here are three writing prompts pulled straight from your audit.`,
    kind:    'write'
  });
  ...
  if (films.length > 0) drawIdeaPage(ctx, r, films, {
    ...
    sub:     `Three video ideas, each shootable on a phone in under two hours. Pick the one that fits your week.`,
    kind:    'video'
  });
}
