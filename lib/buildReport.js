...
function drawWhereYouStand(ctx, r) {
  ...
  // Executive summary — clamped to 3 lines max (was 6). Forces Claude's output
  // to read as a tight headline rather than a paragraph. The lower density
  // matches the "less text" simplification pass.
  ...
  drawWrappedClamped(page, summary, {
    ...
    size: 13, font: ctx.helv, color: COLOR.ink, lineHeight: 19,
    maxLines: 3
  });
...
function drawSixSignals(ctx, r) {
  ...
  // Tightened layout: each dimension gets ONE line of commentary (truncated to
  // ~88 chars). Was wrapping to 2-3 lines which made the page text-dense.
  // Spacing between rows reduced from 26 → 14.
  ...
  DIMENSIONS.forEach((d, i) => {
    ...
    page.drawRectangle({ x: MARGIN + 36, y: barY, width: ((CONTENT_W - 90) * score) / 3, height: 4,
      color: COLOR.indigo });
    // ONE-LINE commentary (truncated). Was multi-line wrapped paragraph.
    const text = truncateOneLine(commentary[d.key] || d.plain, 88);
    drawText(page, text, {
      x: MARGIN + 36, y: y - 28, size: 11, font: ctx.helv, color: COLOR.ink
    });
    y -= 56;  // tighter row spacing (was variable, ~75-90px)
  });
